...
function HomeScreen() {
    ...
    const fetchCompletedTasks = async () => {
        try {
            const userCompleted = db.check.find(user => user.userId === studentId[0]);
                        ...
                        {datesFromMonth.map((date) => {
                            ...
                            return (
                                <HabitDay
                                    ...
                                    future={isFuture}
                                    onPress={() => {
                                        goTo(`/(tabs)/${id}/guardian/${studentId}/habit?date=${date.toISOString()}`);
                                    }}
                                />
                            );
                        })}
                    </View>
                    <TouchableOpacity onPress={() => goTo(`/(tabs)/${id}/guardian/${studentId}/${routineId}`)} className=" bg-green-600 p-4 rounded-lg flex items-center justify-center mt-10">
